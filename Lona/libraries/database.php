<?php 
    class Database
    {
        ...
        {
            $this->link = mysqli_connect("localhost","root","","Lona") or die ();
        ...
        public function count_total_category($connect)
            {
                $query = "
                SELECT * FROM category WHERE category_status='active'
                ";
                
                $statement = mysqli_query($this->link  , $query);
                $statement->execute();
                return $statement->rowCount();
            }

            public function countCategory($table)
        {
